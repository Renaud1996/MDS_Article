...
Max_bins_Nas=max(bins_df$pct_NA)
  #### Print everything
  if (print_only == TRUE) {
  ...
  if (print_only == FALSE) {
    output <- list(
      ...
      percentage_NAs = pct_NAs,
      number_na_gaps = number_gaps,
      average_size_na_gaps = average_gapsize,
      Bins_with_NAs_max=which(bins_df$pct_NA==max(bins_df$pct_NA),arr.ind = T),
      Max_pct_NA=Max_bins_Nas,
      longest_na_gap = longest_NA,
      most_frequent_na_gap = common_NA,
      times_most_frequent_na_gap = common_NAnum,
